...
#define STRINGZILLA_VERSION_PATCH 9
...
typedef enum { sz_false_k = 0,
    sz_true_k = 1 } sz_bool_t; // Only one relevant bit
typedef enum { sz_less_k = -1,
    sz_equal_k = 0,
    sz_greater_k = 1 } sz_ordering_t; // Only three possible states: <=>
...
/**
 *  @brief  Locates first matching byte in a string regardless of case
 *
 *  @param haystack Haystack - the string to search in.
 *  @param h_length Number of bytes in the haystack.
 *  @param needle   Needle - single-byte substring to find.
 *  @return         Address of the first match.
 */
SZ_DYNAMIC sz_cptr_t sz_find_byte_case_insensitive(sz_cptr_t haystack, sz_size_t h_length, sz_cptr_t needle);

/** @copydoc sz_find_byte_case_insensitive */
SZ_PUBLIC sz_cptr_t sz_find_byte_case_insensitive_serial(sz_cptr_t haystack, sz_size_t h_length, sz_cptr_t needle);

/**
 *  @brief Locates first matching substring regardless of case
 *         i.e. HeLlO matches hello
 *
 *  @param haystack Haystack - the string to search in.
 *  @param h_length Number of bytes in the haystack.
 *  @param needle   Needle - substring to find.
 ...
 *  @return         Address of the first match.
 */
SZ_DYNAMIC sz_cptr_t sz_find_case_insensitive(sz_cptr_t haystack, sz_size_t h_length, sz_cptr_t needle, sz_size_t n_length);

/** @copydoc sz_find_case_insensitive
 *
 * This is implemented in `stringzilla.zig`
 */
SZ_PUBLIC sz_cptr_t sz_find_case_insensitive_serial(sz_cptr_t haystack, sz_size_t h_length, sz_cptr_t needle, sz_size_t n_length);
...
/** @copydoc sz_find_byte_case_insensitive */
SZ_PUBLIC sz_cptr_t sz_find_byte_case_insensitive_avx512(sz_cptr_t haystack, sz_size_t h_length, sz_cptr_t needle);
/** @copydoc sz_find_case_insensitive */
SZ_PUBLIC sz_cptr_t sz_find_case_insensitive_avx512(sz_cptr_t haystack, sz_size_t h_length, sz_cptr_t needle, sz_size_t n_length);
...
#define sz_assert(condition)                                    \
    do {                                                        \
        if (!(condition)) {                                     \
            _sz_assert_failure(#condition, __FILE__, __LINE__); \
        }                                                       \
    } while (0)
#else
    ...
    sz_size_t* normalized_offset, sz_size_t* normalized_length)
{
    ...
    if (start > (sz_ssize_t)length)
        start = length;
    if (end > (sz_ssize_t)length)
        end = length;
...
SZ_INTERNAL sz_ptr_t _sz_memory_allocate_fixed(sz_size_t length, void* handle)
{
    ...
    if (consumed_capacity + length > capacity)
        return SZ_NULL_CHAR;
    ...
    sz_size_t* first, sz_size_t* second, sz_size_t* third)
{
    ...
    *third = length - 1;
    ...
    if (length > 8) {
        ...
        for (; (start_u8[vibrant_second] > 191 || start_u8[vibrant_second] == start_u8[vibrant_third]) && (vibrant_second + 1 < vibrant_third);
             ++vibrant_second) { }

        // Now check if we've indeed found a good candidate or should revert the `vibrant_second` to `second`.
        if (start_u8[vibrant_second] < 191) {
            *second = vibrant_second;
        } else {
            vibrant_second = *second;
        }

        // Now check the first character.
        for (; (start_u8[vibrant_first] > 191 || start_u8[vibrant_first] == start_u8[vibrant_second] || start_u8[vibrant_first] == start_u8[vibrant_third]) && (vibrant_first + 1 < vibrant_second);
             ++vibrant_first) { }
...
SZ_PUBLIC sz_cptr_t sz_find_charset_serial(sz_cptr_t text, sz_size_t length, sz_charset_t const* set)
{
    for (sz_cptr_t const end = text + length; text != end; ++text)
        if (sz_charset_contains(set, *text))
            return text;
    return SZ_NULL_CHAR;
}
...
#pragma GCC diagnostic ignored "-Warray-bounds"
    ...
    for (text += length; text != end;)
        if (sz_charset_contains(set, *(text -= 1)))
            return text;
    return SZ_NULL_CHAR;
...
SZ_PUBLIC sz_cptr_t sz_find_byte_serial(sz_cptr_t h, sz_size_t h_length, sz_cptr_t n)
{
    ...
    for (; h + 8 <= h_end; h += 8) {
        ...
        if (match_vec.u64)
            return h + sz_u64_ctz(match_vec.u64) / 8;
    }
#endif
...
sz_cptr_t sz_rfind_byte_serial(sz_cptr_t h, sz_size_t h_length, sz_cptr_t n)
{
    ...
    for (; h >= h_start + 7; h -= 8) {
        ...
        match_vec = _sz_u64_each_byte_equal(h_vec, n_vec);
        if (match_vec.u64)
            return h - sz_u64_clz(match_vec.u64) / 8;
    }
#endif
...
SZ_INTERNAL sz_cptr_t _sz_find_4byte_serial(sz_cptr_t h, sz_size_t h_length, sz_cptr_t n)
{
    ...
    for (; ((sz_size_t)h & 7ull) && h + 4 <= h_end; ++h)
        if ((h[0] == n[0]) + (h[1] == n[1]) + (h[2] == n[2]) + (h[3] == n[3]) == 4)
            return h;
#endif
...
SZ_INTERNAL sz_cptr_t _sz_find_3byte_serial(sz_cptr_t h, sz_size_t h_length, sz_cptr_t n)
{
    ...
    for (; h + sizeof(sz_u64_t) + sizeof(sz_u16_t) <= h_end; h += sizeof(sz_u64_t)) {
        ...
        if (matches0_vec.u64 | matches1_vec.u64 | matches2_vec.u64 | matches3_vec.u64 | matches4_vec.u64) {
            ...
            matches3_vec.u64 <<= 8;
            matches4_vec.u64 <<= 16;
            sz_u64_t match_indicators = matches0_vec.u64 | matches1_vec.u64 | matches2_vec.u64 | matches3_vec.u64 | matches4_vec.u64;
            return h + sz_u64_ctz(match_indicators) / 8;
        }
    }
    ...
    sz_cptr_t n_chars, sz_size_t n_length)
{
    ...
    sz_u8_t const* n = (sz_u8_t const*)n_chars;
    {
        ...
        for (sz_size_t i = 0; i != 64; ++i)
            bad_shift_table.vecs[i].u64 = n_length_vec.u64;
    ...
    for (sz_size_t i = 0; i <= h_length - n_length;) {
        ...
        if (h_vec.u32 == n_vec.u32 && sz_equal((sz_cptr_t)h + i, n_chars, n_length))
            return (sz_cptr_t)h + i;
    ...
    sz_find_t find_prefix, sz_size_t prefix_length)
{
    ...
    while (1) {
        ...
        if (remaining < n_length)
            return SZ_NULL_CHAR;
        if (sz_equal(found + prefix_length, n + prefix_length, suffix_length))
            return found;
    ...
    sz_find_t find_suffix, sz_size_t suffix_length)
{
    ...
    while (1) {
        ...
        if (remaining < prefix_length)
            return SZ_NULL_CHAR;
        if (sz_equal(found - prefix_length, n, prefix_length))
    ...
    sz_size_t bound, sz_memory_allocator_t* alloc)
{
    ...
    for (; next_skew_diagonal_index != total_diagonals; ++next_skew_diagonal_index) {
        ...
        for (sz_size_t i = 0; i != next_skew_diagonal_length; ++i) {
            sz_size_t cost_of_substitution = shorter[shorter_length - 1 - i] != longer[next_skew_diagonal_index - n + i];
            sz_size_t cost_if_substitution = previous_distances[i] + cost_of_substitution;
            sz_size_t cost_if_deletion_or_insertion = sz_min_of_two(current_distances[i], current_distances[i + 1]) + 1;
...
SZ_INTERNAL sz_size_t _sz_export_utf8_to_utf32(sz_cptr_t utf8, sz_size_t utf8_length, sz_rune_t* utf32)
{
    ...
    for (; utf8 != end; utf8 += rune_length, utf32++, count++)
        _sz_extract_utf8_rune(utf8, utf32, &rune_length);
    return count;
}
    ...
    sz_size_t bound, sz_bool_t can_be_unicode, sz_memory_allocator_t* alloc)
{
    ...
    if ((can_be_unicode == sz_true_k) && (sz_isascii(longer, longer_length) == sz_false_k || sz_isascii(shorter, shorter_length) == sz_false_k)) {
        buffer_length += (shorter_length + longer_length) * sizeof(sz_rune_t);
    } else {
        can_be_unicode = sz_false_k;
    }
...
#define _wagner_fisher_unbounded(_distance_t, _char_t)                                                             \
    ...
    for (_distance_t idx_shorter = 0; idx_shorter != n; ++idx_shorter)                                             \
...
#define _wagner_fisher_bounded(_distance_t, _char_t)                                                             \
    ...
    if (!bound) {
        if (can_be_unicode == sz_true_k) {
            _wagner_fisher_unbounded(sz_size_t, sz_rune_t);
        } else {
            _wagner_fisher_unbounded(sz_size_t, sz_u8_t);
        }
    } else {
        if (can_be_unicode == sz_true_k) {
            _wagner_fisher_bounded(sz_size_t, sz_rune_t);
        } else {
            _wagner_fisher_bounded(sz_size_t, sz_u8_t);
        }
    }
}
    ...
    sz_size_t bound, sz_memory_allocator_t* alloc)
{
    ...
    if (bound) {
        ...
        if (longer_length == 0)
            return sz_min_of_two(shorter_length, bound);
        if (shorter_length == 0)
            return sz_min_of_two(longer_length, bound);
    ...
    if (shorter_length == 0)
        return longer_length; // If no mismatches were found - the distance is zero.
    ...
    sz_memory_allocator_t* alloc)
{
    ...
    // If one of the strings is empty - the edit distance is equal to the length of the other one
    if (longer_length == 0)
        return (sz_ssize_t)shorter_length * gap;
    if (shorter_length == 0)
        return (sz_ssize_t)longer_length * gap;
    ...
    sz_size_t bound)
{
    ...
    for (; a != a_end && distance < bound; ++a, ++b) {
        distance += (*a != *b);
    }
    ...
    sz_size_t bound)
{
    ...
    if (bound) {
        ...
        // If one string has more runes, we need to go through the tail.
        for (; a < a_end; a += a_rune_length, ++distance)
            _sz_extract_utf8_rune(a, &a_rune, &a_rune_length);
        for (; b < b_end; b += b_rune_length, ++distance)
            _sz_extract_utf8_rune(b, &b_rune, &b_rune_length);
    }
    return distance;
}
    ...
    sz_hash_callback_t callback, void* callback_handle)
{
    ...
    if (length < window_length || !window_length)
        return;
    sz_u8_t const* text = (sz_u8_t const*)start;
    sz_u8_t const* text_end = text + length;
...
SZ_PUBLIC void sz_look_up_transform_serial(sz_cptr_t text, sz_size_t length, sz_cptr_t lut, sz_ptr_t result)
{
    ...
    for (; unsigned_text != end; ++unsigned_text, ++unsigned_result)
        *unsigned_result = unsigned_lut[*unsigned_text];
}
...
SZ_PUBLIC void sz_tolower_serial(sz_cptr_t text, sz_size_t length, sz_ptr_t result)
{
    ...
    sz_u8_t const* end = unsigned_text + length;
    for (; unsigned_text != end; ++unsigned_text, ++unsigned_result)
        *unsigned_result = sz_u8_tolower(*unsigned_text);
}
...
SZ_PUBLIC void sz_toupper_serial(sz_cptr_t text, sz_size_t length, sz_ptr_t result)
{
    ...
    sz_u8_t const* end = unsigned_text + length;
    for (; unsigned_text != end; ++unsigned_text, ++unsigned_result)
        *unsigned_result = sz_u8_toupper(*unsigned_text);
}
...
SZ_PUBLIC void sz_toascii_serial(sz_cptr_t text, sz_size_t length, sz_ptr_t result)
{
    ...
    sz_u8_t const* end = unsigned_text + length;
    for (; unsigned_text != end; ++unsigned_text, ++unsigned_result)
        *unsigned_result = *unsigned_text & 0x7F;
}
...
SZ_PUBLIC sz_bool_t sz_isascii_serial(sz_cptr_t text, sz_size_t length)
{
    ...
    for (; h + 8 <= h_end; h += 8) {
        ...
        if (text_vec.u64 & 0x8080808080808080ull)
            return sz_false_k;
    }
    ...
    sz_random_generator_t generator, void* generator_user_data)
{
    ...
    if (alphabet_size == 1)
        sz_fill(result, result_length, *alphabet);
...
SZ_PUBLIC sz_ptr_t sz_string_reserve(sz_string_t* string, sz_size_t new_capacity, sz_memory_allocator_t* allocator)
{
    ...
    if (new_space <= SZ_STRING_INTERNAL_SPACE)
        return string->external.start;
    ...
    if (string_is_external)
        allocator->free(string_start, string_space, allocator->handle);
    return string->external.start;
}
...
SZ_PUBLIC sz_ptr_t sz_string_shrink_to_fit(sz_string_t* string, sz_memory_allocator_t* allocator)
{
    ...
    if (string_space == new_space || !string_is_external)
        return string->external.start;
...
SZ_PUBLIC void sz_fill_serial(sz_ptr_t target, sz_size_t length, sz_u8_t value)
{
    ...
    // In case of long strings, skip unaligned bytes, and then fill the rest in 64-bit chunks.
        ...
        while ((sz_size_t)target & 7ull)
            *(target++) = value;
        while (target + 8 <= end)
            *(sz_u64_t*)target = value64, target += 8;
        while (target != end)
            *(target++) = value;
    }
}
...
#if SZ_USE_MISALIGNED_LOADS
    while (length >= 8)
        *(sz_u64_t*)target = *(sz_u64_t const*)source, target += 8, source += 8, length -= 8;
#endif
    while (length--)
        *(target++) = *(source++);
}
...
#if SZ_USE_MISALIGNED_LOADS
        while (length >= 8)
            *(sz_u64_t*)(target -= 8) = *(sz_u64_t const*)(source -= 8), length -= 8;
#endif
        while (length--)
            *(--target) = *(--source);
    }
...
SZ_PUBLIC sz_size_t sz_partition(sz_sequence_t* sequence, sz_sequence_predicate_t predicate)
{
    ...
    sz_size_t matches = 0;
    while (matches != sequence->count && predicate(sequence, sequence->order[matches]))
        ++matches;
...
SZ_PUBLIC void sz_merge(sz_sequence_t* sequence, sz_size_t partition, sz_sequence_comparator_t less)
{
    ...
    if (!less(sequence, sequence->order[start_b], sequence->order[partition]))
        return;
    ...
    while (start_a <= partition && start_b <= sequence->count) {
        ...
        if (!less(sequence, sequence->order[start_b], sequence->order[start_a])) {
            start_a++;
        } else {
            ...
            while (index != start_a) {
                sequence->order[index] = sequence->order[index - 1], index--;
            }
...
SZ_PUBLIC void sz_sort_insertion(sz_sequence_t* sequence, sz_sequence_comparator_t less)
{
    ...
    for (sz_size_t i = 1; i < keys_count; i++) {
        ...
        sz_size_t j = i;
        for (; j > 0 && less(sequence, i_key, keys[j - 1]); --j)
            keys[j] = keys[j - 1];
        keys[j] = i_key;
    }
}
    ...
    sz_size_t end)
{
    ...
    while (2 * root + 1 <= end) {
        ...
        if (child + 1 <= end && less(sequence, order[child], order[child + 1])) {
            child++;
        }
        if (!less(sequence, order[root], order[child])) {
            return;
        }
    ...
    sz_size_t last, sz_size_t depth)
{
    ...
    case 3: {
        ...
        if (less(sequence, b, a))
            sz_u64_swap(&a, &b);
        if (less(sequence, c, b))
            sz_u64_swap(&c, &b);
        if (less(sequence, b, a))
            sz_u64_swap(&a, &b);
    ...
    while (1) {
        while (less(sequence, sequence->order[left], pivot))
            left++;
        while (less(sequence, pivot, sequence->order[right]))
            right--;
        if (left >= right)
            break;
    ...
    sz_size_t partial_order_length)
{
    ...
    for (sz_size_t i = 0; i != sequence->count; ++i)
        count_with_bit_set += (sequence->order[i] & mask) != 0;
    ...
    if (split != 0 && split != sequence->count) {
        ...
        while (1) {
            ...
            while (left < split && !(sequence->order[left] & mask))
                ++left;
            // Find the next element without the bit set on the right side.
            while (right >= split && (sequence->order[right] & mask))
                --right;
...
SZ_PUBLIC void sz_fill_avx2(sz_ptr_t target, sz_size_t length, sz_u8_t value)
{
    ...
    // When the buffer is aligned, we can avoid any split-stores.
        ...
        if (head_length & 1)
            *(sz_u8_t*)target = value, target++, head_length--;
        if (head_length & 2)
            *(sz_u16_t*)target = value16, target += 2, head_length -= 2;
        if (head_length & 4)
            *(sz_u32_t*)target = value32, target += 4, head_length -= 4;
        if (head_length & 8)
            *(sz_u64_t*)target = value64, target += 8, head_length -= 8;
        if (head_length & 16)
        ...
        for (; body_length >= 32; target += 32, body_length -= 32)
            _mm256_store_si256((__m256i*)target, value_vec);
        ...
        if (tail_length & 8)
            *(sz_u64_t*)target = value64, target += 8, tail_length -= 8;
        if (tail_length & 4)
            *(sz_u32_t*)target = value32, target += 4, tail_length -= 4;
        if (tail_length & 2)
            *(sz_u16_t*)target = value16, target += 2, tail_length -= 2;
        if (tail_length & 1)
            *(sz_u8_t*)target = value, target++, tail_length--;
    }
}
...
SZ_PUBLIC void sz_copy_avx2(sz_ptr_t target, sz_cptr_t source, sz_size_t length)
{
    ...
    // and then combine unaligned loads with aligned stores.
        ...
        // Fill the head of the buffer. This part is much cleaner with AVX-512.
        if (head_length & 1)
            *(sz_u8_t*)target = *(sz_u8_t*)source, target++, source++, head_length--;
        if (head_length & 2)
            *(sz_u16_t*)target = *(sz_u16_t*)source, target += 2, source += 2, head_length -= 2;
        if (head_length & 4)
            *(sz_u32_t*)target = *(sz_u32_t*)source, target += 4, source += 4, head_length -= 4;
        if (head_length & 8)
            *(sz_u64_t*)target = *(sz_u64_t*)source, target += 8, source += 8, head_length -= 8;
        if (head_length & 16)
        ...
        if (tail_length & 16)
            _mm_store_si128((__m128i*)target, _mm_lddqu_si128((__m128i const*)source)), target += 16, source += 16,
                tail_length -= 16;
        if (tail_length & 8)
            *(sz_u64_t*)target = *(sz_u64_t*)source, target += 8, source += 8, tail_length -= 8;
        if (tail_length & 4)
            *(sz_u32_t*)target = *(sz_u32_t*)source, target += 4, source += 4, tail_length -= 4;
        if (tail_length & 2)
            *(sz_u16_t*)target = *(sz_u16_t*)source, target += 2, source += 2, tail_length -= 2;
        if (tail_length & 1)
            *(sz_u8_t*)target = *(sz_u8_t*)source, target++, source++, tail_length--;
    }
}
...
SZ_PUBLIC sz_cptr_t sz_rfind_byte_avx2(sz_cptr_t h, sz_size_t h_length, sz_cptr_t n)
{
    ...
    while (h_length >= 32) {
        ...
        mask = _mm256_movemask_epi8(_mm256_cmpeq_epi8(h_vec.ymm, n_vec.ymm));
        if (mask)
            return h + h_length - 1 - sz_u32_clz(mask);
        h_length -= 32;
    }
...
SZ_PUBLIC sz_cptr_t sz_find_avx2(sz_cptr_t h, sz_size_t h_length, sz_cptr_t n, sz_size_t n_length)
{
    ...
    if (h_length < n_length || !n_length)
        return SZ_NULL_CHAR;
    if (n_length == 1)
        return sz_find_byte_avx2(h, h_length, n);
    ...
    for (; h_length >= n_length + 32; h += 32, h_length -= 32) {
        ...
        matches = _mm256_movemask_epi8(_mm256_cmpeq_epi8(h_first_vec.ymm, n_first_vec.ymm)) & _mm256_movemask_epi8(_mm256_cmpeq_epi8(h_mid_vec.ymm, n_mid_vec.ymm)) & _mm256_movemask_epi8(_mm256_cmpeq_epi8(h_last_vec.ymm, n_last_vec.ymm));
        while (matches) {
            int potential_offset = sz_u32_ctz(matches);
            if (sz_equal(h + potential_offset, n, n_length))
...
SZ_PUBLIC sz_cptr_t sz_rfind_avx2(sz_cptr_t h, sz_size_t h_length, sz_cptr_t n, sz_size_t n_length)
{
    ...
    if (h_length < n_length || !n_length)
        return SZ_NULL_CHAR;
    if (n_length == 1)
        return sz_rfind_byte_avx2(h, h_length, n);
    ...
    sz_hash_callback_t callback, void* callback_handle)
{
    ...
    for (; text_fourth != text_end; ++text_first, ++text_second, ++text_third, ++text_fourth, ++cycle) {
        ...
        chars_high_vec.ymm = _mm256_add_epi8(chars_low_vec.ymm, shift_high_vec.ymm);
        hash_low_vec.ymm = _mm256_sub_epi64(hash_low_vec.ymm, _mm256_mul_epu64(chars_low_vec.ymm, prime_power_low_vec.ymm));
        hash_high_vec.ymm = _mm256_sub_epi64(hash_high_vec.ymm, _mm256_mul_epu64(chars_high_vec.ymm, prime_power_high_vec.ymm));
...
SZ_PUBLIC sz_ordering_t sz_order_avx512(sz_cptr_t a, sz_size_t a_length, sz_cptr_t b, sz_size_t b_length)
{
    ...
    } else if (head_length == a_length && head_length == b_length) {
        return sz_equal_k;
    } else {
        a += head_length, b += head_length, a_length -= head_length, b_length -= head_length;
    }
...
{
    if (target == source)
        return; // Don't be silly, don't move the data if it's already there.
    ...
    else if (target + length < source || target >= source + length) {
...
SZ_PUBLIC sz_cptr_t sz_find_avx512(sz_cptr_t h, sz_size_t h_length, sz_cptr_t n, sz_size_t n_length)
{
    ...
    if (h_length < n_length || !n_length)
        return SZ_NULL_CHAR;
    if (n_length == 1)
        return sz_find_byte_avx512(h, h_length, n);
    ...
    if (n_length >= 64) {
        for (; h_length >= n_length + 64; h += 64, h_length -= 64) {
            ...
            while (matches) {
                ...
                if (sz_equal_avx512(h + potential_offset, n, n_length))
                    return h + potential_offset;
                matches &= matches - 1;
            }
    ...
    // The "tail" of the function uses masked loads to process the remaining bytes.
    {
        ...
        while (matches) {
            int potential_offset = sz_u64_ctz(matches);
            if (n_length <= 3 || sz_equal_avx512(h + potential_offset, n, n_length))
                return h + potential_offset;
            matches &= matches - 1;
        }
    }
...
/// Credit: https://dotat.at/@/2024-07-28-tolower-avx512.html
...
SZ_PUBLIC sz_cptr_t sz_find_case_insensitive_avx512(sz_cptr_t h, sz_size_t h_length, sz_cptr_t n, sz_size_t n_length)
{
    // This almost never fires, but it's better to be safe than sorry.
    if (h_length < n_length || !n_length)
        return SZ_NULL_CHAR;
    if (n_length == 1)
        return sz_find_byte_case_insensitive_avx512(h, h_length, n);
    ...
    sz_u512_vec_t h_first_vec, h_mid_vec, h_last_vec, n_first_vec, n_mid_vec, n_last_vec;
    /// PATCH
    n_first_vec.zmm = _mm512_set1_epi8(sz_u8_tolower(n[offset_first]));
    n_mid_vec.zmm = _mm512_set1_epi8(sz_u8_tolower(n[offset_mid]));
    n_last_vec.zmm = _mm512_set1_epi8(sz_u8_tolower(n[offset_last]));
    /// PATCH
    ...
    if (n_length >= 64) {
        for (; h_length >= n_length + 64; h += 64, h_length -= 64) {
            /// PATCH
            h_first_vec.zmm = sz_lower_avx512(_mm512_loadu_si512(h + offset_first));
            h_mid_vec.zmm = sz_lower_avx512(_mm512_loadu_si512(h + offset_mid));
            h_last_vec.zmm = sz_lower_avx512(_mm512_loadu_si512(h + offset_last));
            /// PATCH
            matches = _kand_mask64(_kand_mask64( // Intersect the masks
                                       _mm512_cmpeq_epi8_mask(h_first_vec.zmm, n_first_vec.zmm),
                                       _mm512_cmpeq_epi8_mask(h_mid_vec.zmm, n_mid_vec.zmm)),
    ...
    // slice in 3 CPU cycles.
        ...
        sz_u512_vec_t n_full_vec, h_full_vec;
        /// PATCH
        n_full_vec.zmm = sz_lower_avx512(_mm512_maskz_loadu_epi8(n_mask, n));
        /// PATCH
        for (; h_length >= n_length + 64; h += 64, h_length -= 64) {
            /// PATCH
            h_first_vec.zmm = sz_lower_avx512(_mm512_loadu_si512(h + offset_first));
            ...
            while (matches) {
                int potential_offset = sz_u64_ctz(matches);
                /// PATCH
                h_full_vec.zmm = sz_lower_avx512(_mm512_maskz_loadu_epi8(n_mask, h + potential_offset));
                /// PATCH
                if (_mm512_mask_cmpneq_epi8_mask(n_mask, h_full_vec.zmm, n_full_vec.zmm) == 0)
                    return h + potential_offset;
                matches &= matches - 1;
    ...
    // The "tail" of the function uses masked loads to process the remaining bytes.
    {
        mask = _sz_u64_mask_until(h_length - n_length + 1);
        /// PATCH
        h_first_vec.zmm = sz_lower_avx512(_mm512_maskz_loadu_epi8(mask, h + offset_first));
        h_mid_vec.zmm = sz_lower_avx512(_mm512_maskz_loadu_epi8(mask, h + offset_mid));
        h_last_vec.zmm = sz_lower_avx512(_mm512_maskz_loadu_epi8(mask, h + offset_last));
        /// PATCH
        matches = _kand_mask64(_kand_mask64( // Intersect the masks
                                   _mm512_cmpeq_epi8_mask(h_first_vec.zmm, n_first_vec.zmm),
                                   _mm512_cmpeq_epi8_mask(h_mid_vec.zmm, n_mid_vec.zmm)),
...
SZ_PUBLIC sz_cptr_t sz_rfind_byte_avx512(sz_cptr_t h, sz_size_t h_length, sz_cptr_t n)
{
    ...
    while (h_length >= 64) {
        ...
        mask = _mm512_cmpeq_epi8_mask(h_vec.zmm, n_vec.zmm);
        if (mask)
            return h + h_length - 1 - sz_u64_clz(mask);
        h_length -= 64;
    }
...
SZ_PUBLIC sz_cptr_t sz_rfind_avx512(sz_cptr_t h, sz_size_t h_length, sz_cptr_t n, sz_size_t n_length)
{
    ...
    if (h_length < n_length || !n_length)
        return SZ_NULL_CHAR;
    if (n_length == 1)
        return sz_rfind_byte_avx512(h, h_length, n);
    ...
    for (; h_length >= n_length + 64; h_length -= 64) {
        ...
        while (matches) {
            ...
            if (n_length <= 3 || sz_equal_avx512(h + h_length - n_length - potential_offset, n, n_length))
                return h + h_length - n_length - potential_offset;
            sz_assert((matches & ((sz_u64_t)1 << (63 - potential_offset))) != 0 && "The bit must be set before we squash it");
    ...
    sz_size_t bound, sz_memory_allocator_t* alloc)
{
    ...
    for (sz_size_t i = 0; i != shorter_length; ++i)
        shorter_reversed[i] = shorter[shorter_length - 1 - i];
    ...
    for (; next_skew_diagonal_index != total_diagonals; ++next_skew_diagonal_index) {
        ...
        for (sz_size_t i = 0; i != next_skew_diagonal_length;) {
            ...
            sz_u32_t register_length = remaining_length < 32 ? remaining_length : 32;
            sz_u32_t remaining_length_mask = _bzhi_u32(0xFFFFFFFFu, register_length);
            longer_vec.ymms[0] = _mm256_maskz_loadu_epi8(remaining_length_mask, longer + next_skew_diagonal_index - n + i);
...
SZ_PUBLIC void sz_look_up_transform_avx512(sz_cptr_t source, sz_size_t length, sz_cptr_t lut, sz_ptr_t target)
{
    ...
    if (head_length) {
        ...
        blended_0_to_127_vec.zmm = _mm512_mask_blend_epi8(second_bit_mask, lookup_0_to_63_vec.zmm, lookup_64_to_127_vec.zmm);
        blended_128_to_255_vec.zmm = _mm512_mask_blend_epi8(second_bit_mask, lookup_128_to_191_vec.zmm, lookup_192_to_255_vec.zmm);
        blended_0_to_255_vec.zmm = _mm512_mask_blend_epi8(first_bit_mask, blended_0_to_127_vec.zmm, blended_128_to_255_vec.zmm);
...
SZ_PUBLIC sz_cptr_t sz_find_charset_avx512(sz_cptr_t text, sz_size_t length, sz_charset_t const* filter)
{
    ...
    while (length) {
        ...
        if (matches_mask) {
            ...
            text += load_length, length -= load_length;
        }
    }
    ...
    sz_error_cost_t const* subs, sz_error_cost_t gap, sz_memory_allocator_t* alloc)
{
    ...
    for (sz_size_t idx_shorter = 0; idx_shorter != shorter_length; ++idx_shorter) {
        ...
        for (sz_size_t idx_longer = 0; idx_longer < longer_length; idx_longer += 64) {
            ...
            __mmask64 is_second_or_fourth = _mm512_mask_test_epi8_mask(mask, longer_vec.zmm, is_second_or_fourth_vec.zmm);
            ...
            if (register_length > 16) {
                ...
                cost_deletion_vec.zmm = _mm512_maskz_loadu_epi32((__mmask16)mask, previous_distances + 1 + idx_longer + 16);
                cost_deletion_vec.zmm = _mm512_add_epi32(cost_deletion_vec.zmm, gap_vec.zmm);
                current_vec.zmm = _mm512_max_epi32(cost_substitution_vec.zmm, cost_deletion_vec.zmm);
            ...
            if (register_length > 32) {
                mask = _kshiftri_mask64(mask, 16);
                cost_substitution_vec.zmm = _mm512_maskz_loadu_epi32((__mmask16)mask, previous_distances + idx_longer + 32);
                cost_substitution_vec.zmm = _mm512_add_epi32(
                    cost_substitution_vec.zmm, _mm512_cvtepi16_epi32(_mm512_extracti64x4_epi64(current_32_63_vec, 0)));
                cost_deletion_vec.zmm = _mm512_maskz_loadu_epi32((__mmask16)mask, previous_distances + 1 + idx_longer + 32);
                cost_deletion_vec.zmm = _mm512_add_epi32(cost_deletion_vec.zmm, gap_vec.zmm);
                current_vec.zmm = _mm512_max_epi32(cost_substitution_vec.zmm, cost_deletion_vec.zmm);
            ...
            if (register_length > 48) {
                mask = _kshiftri_mask64(mask, 16);
                cost_substitution_vec.zmm = _mm512_maskz_loadu_epi32((__mmask16)mask, previous_distances + idx_longer + 48);
                cost_substitution_vec.zmm = _mm512_add_epi32(
                    cost_substitution_vec.zmm, _mm512_cvtepi16_epi32(_mm512_extracti64x4_epi64(current_32_63_vec, 1)));
                cost_deletion_vec.zmm = _mm512_maskz_loadu_epi32((__mmask16)mask, previous_distances + 1 + idx_longer + 48);
                cost_deletion_vec.zmm = _mm512_add_epi32(cost_deletion_vec.zmm, gap_vec.zmm);
                current_vec.zmm = _mm512_max_epi32(cost_substitution_vec.zmm, cost_deletion_vec.zmm);
...
SZ_PUBLIC void sz_look_up_transform_neon(sz_cptr_t source, sz_size_t length, sz_cptr_t lut, sz_ptr_t target)
{
    ...
    for (; head_length; target += 1, source += 1, head_length -= 1)
        *target = lut[*(sz_u8_t const*)source];
    ...
    for (; tail_length; target += 1, source += 1, tail_length -= 1)
        *target = lut[*(sz_u8_t const*)source];
}
...
SZ_PUBLIC sz_cptr_t sz_rfind_byte_neon(sz_cptr_t h, sz_size_t h_length, sz_cptr_t n)
{
    ...
    while (h_length >= 16) {
        ...
        matches = _sz_vreinterpretq_u8_u4(matches_vec.u8x16);
        if (matches)
            return h + h_length - 1 - sz_u64_clz(matches) / 4;
        h_length -= 16;
    }
...
SZ_PUBLIC sz_cptr_t sz_find_neon(sz_cptr_t h, sz_size_t h_length, sz_cptr_t n, sz_size_t n_length)
{
    ...
    if (h_length < n_length || !n_length)
        return SZ_NULL_CHAR;
    if (n_length == 1)
        return sz_find_byte_neon(h, h_length, n);
    ...
    if (n_length == 2) {
        ...
        for (; h_length >= 17; h += 16, h_length -= 16) {
            ...
            matches_vec.u8x16 = vandq_u8(vceqq_u8(h_first_vec.u8x16, n_first_vec.u8x16), vceqq_u8(h_last_vec.u8x16, n_last_vec.u8x16));
            matches = _sz_vreinterpretq_u8_u4(matches_vec.u8x16);
            if (matches)
                return h + sz_u64_ctz(matches) / 4;
        }
...
SZ_PUBLIC sz_cptr_t sz_rfind_neon(sz_cptr_t h, sz_size_t h_length, sz_cptr_t n, sz_size_t n_length)
{
    ...
    if (h_length < n_length || !n_length)
        return SZ_NULL_CHAR;
    if (n_length == 1)
        return sz_rfind_byte_neon(h, h_length, n);
    ...
    for (; h_length >= n_length + 16; h_length -= 16) {
        ...
        while (matches) {
            ...
            if (sz_equal(h + h_length - n_length - potential_offset, n, n_length))
                return h + h_length - n_length - potential_offset;
            sz_assert((matches & (1ull << (63 - potential_offset * 4))) != 0 && "The bit must be set before we squash it");
...
SZ_DYNAMIC sz_cptr_t sz_find_char_from(sz_cptr_t h, sz_size_t h_length, sz_cptr_t n, sz_size_t n_length)
{
    sz_charset_t set;
    sz_charset_init(&set);
    for (; n_length; ++n, --n_length)
        sz_charset_add(&set, *n);
