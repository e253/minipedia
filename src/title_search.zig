...
pub fn main() !void {
    var ti = try TitleIndex.init(std.heap.page_allocator, ST(std.os.argv[1], 0));
    defer ti.deinit();
    ...
    try ti.search(ST(std.os.argv[2], 0), null);

    ti.unlock();

    const end = std.time.milliTimestamp();

    for (ti.matches.items, 0..) |m, i| {
        std.debug.print("{}. '{s}'\n", .{ i + 1, m.title });
    }
    std.debug.print("{} matches in {}ms\n", .{ ti.matches.items.len, end - start });
}
