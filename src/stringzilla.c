...
typedef struct sz_implementations_t {
    ...
    sz_find_t find_case_insensitive;
    sz_find_byte_t find_byte_case_insensitive;
...
static void sz_dispatch_table_init(void)
{
    ...
    impl->find_case_insensitive = sz_find_case_insensitive_serial;
    impl->find_byte_case_insensitive = sz_find_byte_case_insensitive_serial;
    ...
    if (caps & sz_cap_x86_avx512f_k) {
        ...
        impl->find_case_insensitive = sz_find_case_insensitive_avx512;
        impl->find_byte_case_insensitive = sz_find_byte_case_insensitive_avx512;

        impl->edit_distance = sz_edit_distance_avx512;
    }

    if ((caps & sz_cap_x86_avx512f_k) && (caps & sz_cap_x86_avx512vl_k) && (caps & sz_cap_x86_avx512vbmi2_k) && (caps & sz_cap_x86_avx512bw_k) && (caps & sz_cap_x86_avx512vbmi_k)) {
...
{
    return sz_dispatch_table.find_case_insensitive(haystack, h_length, needle, n_length);
}

SZ_DYNAMIC sz_cptr_t sz_find_byte_case_insensitive(sz_cptr_t haystack, sz_size_t h_length, sz_cptr_t needle)
{
    return sz_dispatch_table.find_byte_case_insensitive(haystack, h_length, needle);
}
...
#if !SZ_AVOID_LIBC
    if (!generator)
        generator = _sz_random_generator;
#endif
